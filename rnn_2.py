...
class RNN(object):
	...
	def add_pred_single_batch_train(self):
		...
		with tf.variable_scope(tf.get_variable_scope()):
			...
			encoder_outputs, encoder_final_states = tf.nn.bidirectional_dynamic_rnn(fw_cell, bckwd_cell, x, sequence_length=sequence_length, time_major=True)			
	...
	def add_pred_single_batch_test(self):
		with tf.variable_scope(tf.get_variable_scope(), reuse=True):
			...
			#decoder_outputs, state = tf.contrib.legacy_seq2seq.rnn_decoder(tf.unstack(x, axis=0), encoder_final_states, decoder_cell, loop_function=loop_function)
