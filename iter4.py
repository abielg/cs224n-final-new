...
class RNN(object):
	...
	def add_placeholders(self):
		self.encoder_inputs_placeholder = tf.placeholder(tf.int32, shape=([self.config.batch_size, self.config.max_sentence_len]))
		self.labels_placeholder = tf.placeholder(tf.int32, shape=([self.config.batch_size, self.config.max_sentence_len]))
		...
		self.sequence_placeholder = tf.placeholder(tf.int32, shape=([self.config.batch_size]))

	def create_feed_dict(self, inputs_batch, labels_batch=None, sequence_batch=None, mask_batch=None):
		feed_dict = {
	...
	def add_loss_op(self, preds, print_pred=False):
		...
		ce = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels, logits=preds)
