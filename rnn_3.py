...
class RNN(object):
	...
	def add_pred_single_batch_train(self):
		...
		with tf.variable_scope(tf.get_variable_scope()):
			...
			decoder_cell = tf.contrib.rnn.LSTMCell(self.config.encoder_hidden_size, \
			...
			decoder_outputs, state = tf.contrib.legacy_seq2seq.rnn_decoder(y, encoder_final_states, decoder_cell)
			#decoder_ouputs is a list of max_sen_len elems with tensors of size (batch, output_size)

			W = tf.get_variable("W", shape=[self.config.encoder_hidden_size, self.config.vocab_size], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float64)
			b = tf.get_variable("b", shape=[self.config.vocab_size], initializer=tf.constant_initializer(0.0), dtype=tf.float64)

			projected_outputs = [tf.matmul(output_tensor, W) + b for output_tensor in decoder_outputs]

		return projected_outputs, W, b
	...
	def add_pred_single_batch_test(self, W, b):
		with tf.variable_scope(tf.get_variable_scope(), reuse=True):
			...
			print("decoder_cell output size:")
			print(decoder_cell.output_size)

			print("decoder_cell state size:")
			...
			def loop_function(prev, i):
				projected_prevs = tf.matmul(prev, W) + b
				index = tf.argmax(projected_prevs, axis=1)
				input = self.add_embedding(index)
				return input
			
			#SETTING NUM_HEADS TO 8
			'''
			decoder_outputs, decoder_state = tf.contrib.legacy_seq2seq.attention_decoder(tf.unstack(x, axis=0), encoder_final_states, attention_states,\
											 decoder_cell, output_size=self.config.vocab_size, num_heads=8,\
											 loop_function=loop_function, dtype=tf.float64)
			'''
			decoder_outputs, state = tf.contrib.legacy_seq2seq.rnn_decoder(y, encoder_final_states, decoder_cell, loop_function=loop_function)
